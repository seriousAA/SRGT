...
class RotatedVariedSizeWindowAttention_KVDIFF(nn.Module):
    def __init__(self, dim, num_heads, out_dim=None, window_size=1, qkv_bias=True, qk_scale=None, 
        ...
        if self.learnable:
            ...
            self.sampling_offsets_k = nn.Sequential(
            ...
            self.sampling_scales_k = nn.Sequential(
                nn.AvgPool2d(kernel_size=window_size, stride=window_size), 
                nn.LeakyReLU(), 
                nn.Conv2d(dim, self.num_heads * self.num_deform * 2, kernel_size=1, stride=1)
            )
            # add angle
            self.sampling_angles_k = nn.Sequential(
                nn.AvgPool2d(kernel_size=window_size, stride=window_size), 
                nn.LeakyReLU(), 
                nn.Conv2d(dim, self.num_heads * self.num_deform * 1, kernel_size=1, stride=1)
            )
            
            self.sampling_offsets_v = nn.Sequential(
                nn.AvgPool2d(kernel_size=window_size, stride=window_size),
                nn.LeakyReLU(), 
                nn.Conv2d(dim, self.num_heads * self.num_deform * 2, kernel_size=1, stride=1)
            )
            self.sampling_scales_v = nn.Sequential(
                nn.AvgPool2d(kernel_size=window_size, stride=window_size), 
                nn.LeakyReLU(), 
                nn.Conv2d(dim, self.num_heads * self.num_deform * 2, kernel_size=1, stride=1)
            )
            # add angle
            self.sampling_angles_v = nn.Sequential(
                nn.AvgPool2d(kernel_size=window_size, stride=window_size), 
                nn.LeakyReLU(), 
                nn.Conv2d(dim, self.num_heads * self.num_deform * 1, kernel_size=1, stride=1)
            )
...
#            self.shift_size = 0
    ...
    def forward(self, x, H, W):
        ...
        if self.restart_regression:
            ...
            coords_k = base_coords.repeat(b*self.num_heads, 1, 1, 1, 1, 1) # B*nH, 2, nh, ws, nw, ws
            coords_v = base_coords.repeat(b*self.num_heads, 1, 1, 1, 1, 1) # B*nH, 2, nh, ws, nw, ws
        if self.learnable:
            # kvdiff --- offset factors
            sampling_offsets_k = self.sampling_offsets_k(x)
            sampling_offsets_v = self.sampling_offsets_v(x)
            
            num_predict_total = b * self.num_heads * self.num_deform
            
            sampling_offsets_k = sampling_offsets_k.reshape(num_predict_total, 2, window_num_h, window_num_w)
            sampling_offsets_v = sampling_offsets_v.reshape(num_predict_total, 2, window_num_h, window_num_w)
            sampling_offsets_k[:, 0, ...] = sampling_offsets_k[:, 0, ...] / (h // self.ws)
            sampling_offsets_k[:, 1, ...] = sampling_offsets_k[:, 1, ...] / (w // self.ws)
            
            sampling_offsets_v[:, 0, ...] = sampling_offsets_v[:, 0, ...] / (h // self.ws)
            sampling_offsets_v[:, 1, ...] = sampling_offsets_v[:, 1, ...] / (w // self.ws)
            
            # kvdiff --- scale factors
            sampling_scales_k = self.sampling_scales_k(x)       #B, heads*2, h // window_size, w // window_size
            sampling_scales_k = sampling_scales_k.reshape(num_predict_total, 2, window_num_h, window_num_w)
            
            sampling_scales_v = self.sampling_scales_v(x)       #B, heads*2, h // window_size, w // window_size
            sampling_scales_v = sampling_scales_v.reshape(num_predict_total, 2, window_num_h, window_num_w)

            # kvdiff --- rotate factors

            sampling_angle_k = self.sampling_angles_k(x)
            ...
            window_coords_k = window_coords * (sampling_scales_k[:, :, :, None, :, None] + 1)
            window_coords_v = window_coords * (sampling_scales_v[:, :, :, None, :, None] + 1)

            # then rotate around window center

            window_coords_r_k = window_coords_k.clone()
            window_coords_r_v = window_coords_v.clone()

            # 0:x,column, 1:y,row

            window_coords_r_k[:,0,:,:,:,:] = -window_coords_k[:,1,:,:,:,:]*torch.sin(sampling_angle_k[:,0,:,None,:,None]) + window_coords_k[:,0,:,:,:,:]*torch.cos(sampling_angle_k[:,0,:,None,:,None])
            window_coords_r_k[:,1,:,:,:,:] = window_coords_k[:,1,:,:,:,:]*torch.cos(sampling_angle_k[:,0,:,None,:,None]) + window_coords_k[:,0,:,:,:,:]*torch.sin(sampling_angle_k[:,0,:,None,:,None])
            
            window_coords_r_v[:,0,:,:,:,:] = -window_coords_v[:,1,:,:,:,:]*torch.sin(sampling_angle_v[:,0,:,None,:,None]) + window_coords_v[:,0,:,:,:,:]*torch.cos(sampling_angle_v[:,0,:,None,:,None])
            window_coords_r_v[:,1,:,:,:,:] = window_coords_v[:,1,:,:,:,:]*torch.cos(sampling_angle_v[:,0,:,None,:,None]) + window_coords_v[:,0,:,:,:,:]*torch.sin(sampling_angle_v[:,0,:,None,:,None])

            # system transformation: window center -> image center
            
            coords_k = window_reference + window_coords_r_k + sampling_offsets_k[:, :, :, None, :, None]
            coords_v = window_reference + window_coords_r_v + sampling_offsets_v[:, :, :, None, :, None]

        # final offset
        sample_coords_k = coords_k.permute(0, 2, 3, 4, 5, 1).reshape(num_predict_total, self.attn_ws*window_num_h, self.attn_ws*window_num_w, 2)
        sample_coords_v = coords_v.permute(0, 2, 3, 4, 5, 1).reshape(num_predict_total, self.attn_ws*window_num_h, self.attn_ws*window_num_w, 2)
        ...
        k_selected = F.grid_sample(
                        ...
                        grid=sample_coords_k, padding_mode='zeros', align_corners=True
        ...
        v_selected = F.grid_sample(
                        ...
                        grid=sample_coords_v, padding_mode='zeros', align_corners=True
                        ).reshape(b*self.num_heads, self.out_dim // self.num_heads, h+padding_td, w+padding_lr)
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if tokens_type == 'transformer':
            if not window:
                ...
                self.attn = RotatedVariedSizeWindowAttention_KVDIFF(
...
class ViTAE_NC_Win_RVSA_V3_KVDIFF_WSZ7(nn.Module):
