...
def log_variables(v, tag: str, interval=500, filename=None):
    """Log the variables when training

    Args:
        v (dict(str,tensor)): some variables want to record
    """
    assert isinstance(v,dict), "only dict type of variables are accepted to saved in pth file!"
    rank, _ = get_dist_info()
    if rank != 0:
        return
    # _, key = _find_caller()
    _log_counter[tag] += 1
    if not (interval == 1 or _log_counter[tag] % interval == 1):
        return
    if filename is None:
        filename = f"{_log_counter[tag]}.pth"

    root_dir = os.environ.get("WORK_DIR", ".")
    save_dir = os.path.join(root_dir, tag)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    torch.save(v,os.path.join(save_dir,filename))