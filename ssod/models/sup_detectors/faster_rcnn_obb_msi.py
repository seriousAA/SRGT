from mmdet.models import OBBTwoStageDetector, DETECTORS, build_detector
import torch
...
class MSInputFasterRCNNOBB(OBBTwoStageDetector):
    def __init__(self, divisor=32, test_on='fr', kd_alpha=0., kd_t=3, do_reduce=True,
        ...
        super(MSInputFasterRCNNOBB, self).__init__(*args, **kwargs)
    ...
    def resize_info(self, hr_info, scale=0.5):
        ...
        for key in ['gt_bboxes', 'gt_bboxes_ignore', 'gt_obboxes', 'gt_obboxes_ignore']:
            if hr_info.get(key, None):
                for i in range(len(ret_info[key])):
                    ...
                    ret_info[key][i][:, :4] *= scale_factor.type_as(ret_info[key][i])
    ...
    def forward_train(self,
        ...
        """
        super(OBBTwoStageDetector, self).forward_train(img, img_metas)

        hr_info = {'img': img, 'img_metas': img_metas,
                   'gt_bboxes': gt_bboxes, 'gt_bboxes_ignore': gt_bboxes_ignore,
                   'gt_obboxes': gt_obboxes, 'gt_obboxes_ignore': gt_obboxes_ignore,
    ...
    def multi_res_losses(self, multi_res_info, **kwargs):
        ...
        for res_key, info in multi_res_info.items():
            ...
            gt_obboxes, gt_obboxes_ignore = info['gt_obboxes'], info['gt_obboxes_ignore']
