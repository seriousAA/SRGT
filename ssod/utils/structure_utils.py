import warnings
try:
    from collections.abc import Mapping, Sequence  # Python 3.10 and above
except ImportError:
    from collections import Mapping, Sequence       # Python 3.9 and below
...
def weighted_loss(loss: dict, weight, ignore_keys=[], warmup=0):
    ...
    elif isinstance(weight, Number) or hasattr(weight, 'item'):
        # Convert tensor to number if needed
        weight_value = weight.item() if hasattr(weight, 'item') else weight
        for name, loss_item in loss.items():
            if "loss" in name:
                if not is_match(name, ignore_keys):
                    loss[name] = sequence_mul(loss[name], lambda_weight(weight_value))
                else:
