...
class MSISubModulesEvalHook(EvalHook):
    def __init__(self, *args, evaluated_modules=None, test_scales=None, **kwargs):
    ...
    def _do_evaluate(self, runner):
        ...
        for submodule in submodules:
            ...
            prefix = submodule
            submodel_ref = getattr(model_ref, submodule)
            curr_test_on = submodel_ref.test_on
            for test_on in self.test_scales:
                prefix += f'.{test_on}'
                submodel_ref.test_on = test_on
                ...
                key_score = self.evaluate(runner, results, prefix=prefix)
                if key_score is not None:
                    key_scores.append(key_score)
            submodel_ref.test_on = curr_test_on
...
@HOOKS.register_module()
class DistMSISubModulesEvalHook(DistEvalHook):
    def __init__(self, *args, evaluated_modules=None, test_scales=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.evaluated_modules = evaluated_modules
        self.test_scales = test_scales
