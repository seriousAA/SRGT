...
class GeometricTransformationBase(object):
    ...
    def _get_flip_matrix(cls, shape, direction="horizontal", inverse=False):
        h, w = shape
        if direction == "horizontal":
        # For horizontal flip, negate x-coordinates and translate by width minus one
            flip_matrix = np.float32([[-1, 0, w - 1], [0, 1, 0], [0, 0, 1]])
        else:
            # For vertical flip, negate y-coordinates and translate by height minus one
            flip_matrix = np.float32([[1, 0, 0], [0, -1, h - 1], [0, 0, 1]])
        return flip_matrix
