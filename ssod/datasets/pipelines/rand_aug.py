...
from mmdet.datasets.pipelines.obb.base import OBBRandomFlip, RandomOBBRotate
...
class RandAug(object):
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(prob={self.prob}, magnitude={self.magnitude}, \
            max_magnitude={self.magnitude_limit}, random_magnitude={self.random_magnitude})"
...
class Sequential(BaseCompose):
    ...
    def enable_record(self, mode: bool = True):
        ...
        for transform in self.transforms:
            if hasattr(transform, 'enable_record'):
...
@PIPELINES.register_module()
class hvRandomFlipOBB(OBBRandomFlip):
...
@PIPELINES.register_module()
class RandRotate(RandomOBBRotate):
    ...
    def __call__(self, results):
        ...
        if self.record:
            if 'matrix' not in results:
                UserWarning("Matrix not found in results, use identity matrix instead")
                GTrans.apply(results, "rotate", cv2_rotation_matrix=np.eye(3)[:2])
                return results
            GTrans.apply(results, "rotate", cv2_rotation_matrix=results['matrix'])
            results.pop('matrix')
