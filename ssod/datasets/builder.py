...
from mmcv.utils import Registry, build_from_cfg, TORCH_VERSION, digit_version
from mmcv.utils import Registry, build_from_cfg
from mmdet.datasets.builder import worker_init_fn, build_dataloader as build_dataloader_normal
...
def dataloader_selector(func):
    def wrapper(dataset, **cfg):
        if 'sampler_cfg' in cfg and 'type' in cfg['sampler_cfg']:
            return build_dataloader_with_sampler(dataset, **cfg)
        else:
            cfg.pop('sampler_cfg', None)
            return build_dataloader_normal(dataset, **cfg)
    return wrapper


@dataloader_selector
def build_dataloader(dataset, **cfg):
    # The decorator does the job, so just pass here
    pass


def build_dataloader_with_sampler(
    dataset,
    samples_per_gpu,
    workers_per_gpu,
    num_gpus=1,
    ...
    **kwargs,
    ...
    data_loader = DataLoader(
        ...
        collate_fn=partial(collate, samples_per_gpu=samples_per_gpu,
                           flatten=True) if runner_type == 'IterBasedRunner' else partial(
            mmcv_collate, samples_per_gpu=samples_per_gpu),
        pin_memory=False,
