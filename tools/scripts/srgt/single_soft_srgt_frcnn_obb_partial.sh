...
CONFIG=configs/srgt/softT_faster_rcnn_obb_r50_fpn_dota20_partial.py
...
find_free_gpu() {
	...
	while true; do
		available_gpus=($(nvidia-smi --query-gpu=memory.free --format=csv,noheader,nounits | awk '$1 >= 15000 {print NR-1}'))
...
for percent in "${percent_settings[@]}"; do
    ...
    run_name=${fold}_${percent}_soft_srgt_frcnn_${current_date}
		output_folder=partial_soft_srgt_frcnn_focal_gsd/${run_name}
		...
		CUDA_VISIBLE_DEVICES=$free_gpu \
			python $(dirname "$0")/../../test.py $CONFIG ${work_dir}/latest.pth \
				--format-only --cfg-options run_name=test_$run_name work_dir=$work_dir runner.max_iters=180000 \
		...
		set +x
			...
			manual_sleep 7200 # 2 hours in seconds
