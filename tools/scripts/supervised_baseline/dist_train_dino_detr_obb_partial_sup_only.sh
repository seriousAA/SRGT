...
for percent in "${percent_settings[@]}"; do
	...
	for fold in {2..10..2}; do
		...
		output_folder=partial_dino_detr_sup_only/${fold}_${percent}_dino_detr_sup_only_${current_date}
		...
		CUDA_VISIBLE_DEVICES=2,3 \
		python -m torch.distributed.launch --nproc_per_node=2 --master_port=${PORT:-29515} \
			$(dirname "$0")/../../test.py $CONFIG $pth_file --launcher=pytorch \
				--format-only --cfg-options test_cfg.max_per_img=2000 \
